...
This document tracks the planned work for LumenLink. The default transport policy is direct peer-to-peer traffic: the public server provides signaling and connection coordination, but does not carry screen, audio, input, or file-transfer traffic unless a future user explicitly enables relay fallback.
...
- [ ] Replace fixed "controller" and "host" startup roles with a unified online-device model.
- [ ] Publish device ID, display name, platform, application version, capabilities, and online state.
- [ ] Implement device key pairing and a trusted-device list.
- [ ] Implement control-session requests, accept/reject flow, disconnect flow, and session expiry.
...
- [ ] Complete Windows-to-Ubuntu and Ubuntu-to-Ubuntu direct connection tests.
- [ ] Show connection state, candidate type, selected path, and failure diagnostics in the client.
...
- [ ] Let every running client appear as an online device rather than a permanently assigned controller or controlled endpoint.
- [ ] Allow either device to initiate a control request by selecting another online device.
- [ ] Allow the same two devices to reverse control direction in a later session without restarting either client.
...
- [ ] Send a WebRTC video track between directly connected peers.
- [ ] Detect and negotiate supported codecs, starting with H.264 and VP8.
- [ ] Support 30, 60, 90, and 120 FPS presets where capture and hardware allow them.
- [ ] Support 2 Mbps, 8 Mbps, and 20 Mbps quality presets plus adaptive bitrate control.
- [ ] Support selecting and switching between multiple displays.
- [ ] Handle standard and non-standard display resolutions, including controller-side scaling and coordinate remapping.
...
- [ ] Implement Windows keyboard and mouse input injection.
- [ ] Implement Ubuntu keyboard and mouse input injection, with separate capability handling for X11 and Wayland.
- [ ] Map mouse coordinates correctly across scaling, window size, display rotation, and non-standard resolutions.
- [ ] Support drag, scroll, multi-button mice, keyboard combinations, and clipboard text synchronization.
...
- [ ] Capture and transmit controlled-device system audio.
...
- [ ] Require explicit authorization for control, audio, clipboard, and file-transfer capabilities.
...
1. Dynamic device model, device pairing, and control-session authorization.
2. Verify direct UDP connectivity across the target Windows and Ubuntu network scenarios.
3. Deliver direct screen-video transport.
4. Deliver keyboard and mouse control.
5. Add persistent background operation and locked-device status handling.
6. Add system audio and independent file transfer.
